...
- **The area is drawn by hand.** Tap to place vertices, drag to adjust, tap the first to close. Administrative boundaries are not how an area is chosen — they answer *are you in the same Bezirk as me*, which is an M6 question, and most real areas agree with no boundary anyway: a Deutschlandticket game, a Ringbahn game, or the bit of Neukölln you like
...
- **A transit catalog for all of Germany**, built once from an open GTFS feed into a 22 MB file the server holds — 251,741 stations, no database. Adding an area is data, not code, and there is nothing to add, because the country is already in there
- Transit inventory materialised onto the map — **every station near the area, with the modes that call there** — so a playing phone never queries the catalog. It feeds place search, the hider's zone commitment, the validity check above and M6's transit questions; it contributes no geometry to the area. Lines and operators are not carried: nothing before M18 groups by them
...
**Reviewable when:** A host draws a ring around central Berlin at a 500 m radius in under two minutes, sees the station count update as they drag a vertex in and out, and shares a code that reproduces the map byte-identically on another device. The same builder produces a district-sized map and a nationwide regional-rail map without the UI collapsing at either end.

**Deferred:** **Hiding radius per mode, enable/disable by mode, line or stop, and the union of station radii that turns them into geometry — one feature, deferred whole (M18).** What that costs is not the station rule, which the validity check above keeps: it is that M13's deduction map is seeded with the whole area rather than with the union, making it coarser and never wrong. Also choosing an area *from* an administrative boundary, which is additive once M6 has the boundary table (M18); exclusion polygons and imports (M18); an automated catalog refresh (M20, where feed freshness first matters); areas beyond Germany (M19).
...
- **The administrative boundary table**, which matching questions need and nothing before this point does. The OSM extract that produces it is already built (`npm run osm:extract`, m4-spec §4); what M6 adds is the artifact it builds into, and the point lookup behind *are you in the same Bezirk/Ortsteil as me*. The level a question means is not `admin_level = 9` — Berlin's Bezirke are level 9, Hamburg's too, and elsewhere that level is a Stadtbezirk of a level-8 Gemeinde
...
- **M4's bottleneck is the data, not the geometry.** With the area reduced to one drawn polygon, building a map is a draw tool and the geometry is free; what is left to get wrong is the catalog, which is one generated file. The German feed classifies every S-Bahn, RE and ICE as one route type, so the mode split is a heuristic over line names — and a heuristic that drifts produces a map which looks entirely plausible and is wrong. It is a labelled table with a test over the real feed for that reason. Administrative boundaries are a second import the feed does not carry at all, but nothing in M4 reads one — they are M6's, and the extract for them is already built and measured. Test the extremes early: one district-sized map and one nationwide map, not just city-sized ones, and test with hosts who did not build the app.
